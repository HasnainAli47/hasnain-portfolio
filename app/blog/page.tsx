  ...
  return (
    <>
      <style>{`
        ...
        .post-card:hover { border-color: rgba(255,197,61,0.45) !important; transform: translateY(-3px); }
      `}</style>
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "64px 24px 120px" }}>
        <p style={{ color: "#FFC53D", fontSize: 11, fontWeight: 800, letterSpacing: "0.18em", textTransform: "uppercase", marginBottom: 14 }}>Writing</p>
        <h1 style={{ fontSize: "clamp(32px, 5vw, 52px)", fontWeight: 900, color: "#EFEBE3", marginBottom: 14, letterSpacing: "-0.03em" }}>AI Engineering Insights</h1>
        <p style={{ fontSize: 17, color: "#7A736B", marginBottom: 64, lineHeight: 1.65 }}>In-depth articles on LLMs, RAG, KAG, and production AI engineering — written from real deployments, not theory.</p>
            ...
            <Link key={post.href} href={post.href} style={{ textDecoration: "none" }}>
              <article className="post-card" style={{ padding: "28px 32px", borderRadius: 20, border: "1px solid #1f1f40", background: "linear-gradient(160deg, #0f0f22 0%, #0a0a18 100%)", cursor: "pointer" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                  ...
                  <span style={{ fontSize: 12, color: "#5F5952" }}>{post.date}</span>
                  <span style={{ fontSize: 12, color: "#5F5952" }}>{post.read} read</span>
                </div>
                <h2 style={{ fontSize: 20, fontWeight: 800, color: "#EFEBE3", marginBottom: 10, letterSpacing: "-0.02em", lineHeight: 1.3 }}>{post.title}</h2>
                <p style={{ fontSize: 14, color: "#7A736B", lineHeight: 1.7, margin: 0 }}>{post.desc}</p>
              </article>
            </Link>
          ))}
        </div>
